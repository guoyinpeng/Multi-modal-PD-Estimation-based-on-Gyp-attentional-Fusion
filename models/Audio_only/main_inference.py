...
def _forward_audio(audio_net, evaluator, data, args):
    B, Fa, Ta = data['audio'].shape
    audio_input = data['audio'].view(B, Fa, Ta)
    feat = audio_net(audio_input.to(args.device))
    return evaluator(feat)


def main(dataloaders, audio_net, evaluator, base_logger, writer, config, args, model_type, ckpt_path, fold_idx=None,
         run_trace_dir=None):
    model_parameters = list(audio_net.parameters()) + list(evaluator.parameters())
        ...
        for mode in ['train', 'val', 'test']:
            ...
            for data in tqdm(dataloaders[mode]):
                ...
                if mode == 'train':
                    ...
                    if config['OPTIMIZER'].get('USE_SAM'):
                        for m in [audio_net, evaluator]:
                            enable_running_stats(m)
                        probs = _forward_audio(audio_net, evaluator, data, args)
                        ...
                        probs2 = _forward_audio(audio_net, evaluator, data, args)
            ...
            if mode == 'val':
                if average_loss < best_val_loss:
                    ...
                    stall = 0
                    save_model_weights(audio_net, evaluator, epoch, model_type, ckpt_path,
...
if __name__ == '__main__':
    ...
    for fold_idx in range(n_splits):
        print('=' * 80)
        print(f'Audio-only | Fold {fold_idx + 1}/{n_splits}')
        print('=' * 80)
        ...
        audio_net, evaluator = get_models(config['MODEL'], args, model_type, fold_ckpt_path)
        mcfg = config['MODEL']
        mcfg.setdefault('EARLY_STOP_PATIENCE', 15)

        fold_result = main(
            dataloaders, audio_net, evaluator, base_logger, writer, mcfg, args,
