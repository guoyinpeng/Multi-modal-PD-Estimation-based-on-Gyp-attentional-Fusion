...
from models.convlstm import (
    ConvLSTM_Audio,
    ConvLSTM_AudioTransformer,
    AudioCNNLSTMBaseline,
)
...
# 获取模型及其权重（仅语音分支 DSFE + PDAF-Net 分类头）
def get_models(model_config, args, model_type=None, ckpt_path=None):
    an = model_config['AUDIO_NET']
    bb = an.get('BACKBONE', 'bilstm').lower()
    common = dict(
        input_dim=an['INPUT_DIM'],
        output_dim=an['OUTPUT_DIM'],
        conv_hidden=an['CONV_HIDDEN'],
    ...
    if bb == 'bilstm':
        audio_net = ConvLSTM_Audio(
            lstm_hidden=an['LSTM_HIDDEN'],
            num_layers=an['NUM_LAYERS'],
            **common
        )
    elif bb == 'transformer':
        audio_net = ConvLSTM_AudioTransformer(
            transformer_dim=an.get('TRANSFORMER_DIM', 256),
            num_layers=an['NUM_LAYERS'],
            dim_feedforward=an.get('DIM_FEEDFORWARD'),
            learnable_pe=an.get('LEARNABLE_PE', True),
            **common
        )
    elif bb == 'cnn_lstm':
        audio_net = AudioCNNLSTMBaseline(
            lstm_hidden=an['LSTM_HIDDEN'],
            num_layers=an['NUM_LAYERS'],
            **common
        )
        ...
        raise ValueError(
            f"不支持的 AUDIO_NET.BACKBONE: {bb}，请使用 'bilstm'、'transformer' 或 'cnn_lstm'"
        )
...
def save_model_weights(audio_net, evaluator, epoch, model_type, ckpt_path, best_metric=None, value=None):
