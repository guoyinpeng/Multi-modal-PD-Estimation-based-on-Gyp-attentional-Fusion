...
from models.convlstm import ConvLSTM_Visual, ConvLSTM_Audio
...
def get_models(model_config, args, model_type=None, ckpt_path=None):
    ...
    # args：包含设备设置的参数
    # model_type（可选）：指定模型类型 可以根据这个参数选择加载特定类型的模型。
    # ckpt_path（可选）：权重文件存储的路径，用于加载模型权重。

    # 创建视觉网络 visual_net，并通过 model_config 中的配置参数初始化该网络。
    vn = model_config['VISUAL_NET']
    visual_net = ConvLSTM_Visual(input_dim=vn['INPUT_DIM'],
    ...
    fn = model_config['FUSION_NET']
    if fn.get('USE_PDAF_PAPER', True):
        fusion_net = PDAFFusion(
            dim=model_config['AUDIO_NET']['OUTPUT_DIM'],
            num_heads=fn.get('NUM_HEADS', 4),
            iaff_r=fn.get('IAFF_R', 4),
        )
    else:
        fusion_net = SimpleConcatFusion(dim=model_config['AUDIO_NET']['OUTPUT_DIM'])

    # 定义评估器网络
    # 评估器网络evaluator用于根据不同输入特征进行最终的预测输出
    ...
    # 加载模型权重
    # WEIGHTS.TYPE 为 'last'，则调用 find_last_ckpts 函数来查找最新的检查点文件，并加载权重
    ...
    # 如果 WEIGHTS.TYPE 为 'absolute_path'，则直接使用配置文件中指定的绝对路径来加载权重文件
    ...
    # 如果 WEIGHTS.TYPE 不为'last'、'absolute_path'也不为 'new'，加载默认路径的权重，这个默认路径通常放的是预训练的权重文件，
    ...
    # 如果 WEIGHTS.TYPE 为 'new'，则表示模型是新的，不需要加载预训练的权重文件。
    else:
        weights_path = None

    # 加载权重并将其应用到模型
    # 根据 WEIGHTS.INCLUDED 字段中指定的模块（如 visual_net、audio_net、evaluator），从权重文件中加载对应的模型权重
    ...
    return visual_net, audio_net, fusion_net, evaluator

# NOTE:保存模型权重
def save_model_weights(visual_net, audio_net, fusion_net, evaluator, epoch, model_type, ckpt_path, best_metric=None,value=None):
    # 检查并创建存储目录
    ...
    # 保存检查点文件，文件名包含模型类型和 epoch
    # 只保存最佳F1score和acc的模型权重
